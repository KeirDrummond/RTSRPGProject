...
bool AProjectPlayerController::AddToSelected(IGameUnit* unit)
{
	...
	if (Cast<AGameCharacter>(theUnit)) { unitArray.Add(unit); }

	IGameUnit::Execute_SetSelected(theUnit, true);
...
bool AProjectPlayerController::RemoveFromSelected(IGameUnit* unit)
{
	...
	if (IsValid(character)) { unitArray.Remove(character); }

	IGameUnit::Execute_SetSelected(unit->_getUObject(), false);
	IGameUnit::Execute_UpdateColour(unit->_getUObject());
